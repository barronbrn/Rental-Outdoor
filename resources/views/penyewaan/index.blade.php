...
@section('content')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    <form action="{{ route('penyewaan.store') }}" method="POST" class="space-y-6">
                        ...
                        <!-- Pilih Peralatan -->
                        <div>
                            <label for="peralatan_id" class="block text-lg font-medium text-gray-700 mb-2">Pilih
                                Peralatan</label>
                            <select name="peralatan_id" id="peralatan_id"
                                ...
                                @foreach ($peralatan as $item)
                                    <option value="{{ $item->id }}">{{ $item->namaPeralatan }} - Rp
                                        {{ number_format($item->harga, 0, ',', '.') }}/hari</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <!-- Durasi Penyewaan -->
                        <div>
                            <label for="durasi" class="block text-lg font-medium text-gray-700 mb-2">Durasi Penyewaan
                                (hari)</label>
                        ...
                        <!-- Metode Pembayaran -->
                        <div>
                            <label for="tanggal_penyewaan" class="block text-lg font-medium text-gray-700 mb-2">Tanggal
                                Penyewaan</label>
                            <input type="text" name="tanggal_penyewaan" id="tanggal_penyewaan"
                        ...
                        <div class="text-center">
                            <button type="submit"
                                class="w-full bg-blue-600 text-white py-3 px-6 rounded-lg hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition duration-200">
                                Sewa Sekarang
                            </button>
                        </div>
                    </form>
                ...
                <div class="bg-gray-50 p-6 rounded-lg">
                    ...
                    <div id="peralatan-detail" class="space-y-4">
                        <!-- Gambar Peralatan -->
                        <img id="peralatan-gambar" src="{{ asset('img/nophoto.jpg') }}" alt="Gambar Peralatan"
            ...
            function updatePeralatanDetail() {
                ...
                if (selectedPeralatan) {
                    ...
                    const fotoPath = selectedPeralatan.foto ?
                        (selectedPeralatan.foto.startsWith('http') ? selectedPeralatan.foto :
                            "{{ asset('') }}" + selectedPeralatan.foto) :
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // Ambil elemen input tanggal penyewaan
        const tanggalPenyewaanInput = document.getElementById('tanggal_penyewaan');
        ...
        // Set nilai input tanggal penyewaan
        tanggalPenyewaanInput.value = formattedDateTime;
    });
    </script>
@endsection
